    ...
    return (
        <div className="min-h-screen relative bg-gradient-to-b from-backgroundDark via-[80%] to-backgroundLight">
            <nav className="fixed top-0 left-0 right-0 z-10 flex items-center justify-between p-4 bg-backgroundDark/80 backdrop-blur-sm" style={{ zIndex: 1 }}>
                <h1 className="text-xl p-1 font-bold text-primary" onClick={() => fetchNews}>based</h1>
                <select
            ...
            {podcast && (
                <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
                    <div className="bg-backgroundDark p-6 rounded-lg max-w-2xl w-[85vw] mx-4 shadow-2xl">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-xl font-bold text-primary">{podcast.title}</h2>
                            <button
                                onClick={closePodcast}
                                className="text-gray-500 font-3xl hover:text-gray-300"
                            >
                                ✕
                            </button>
                        </div>
                        <p className="text-gray-300 mb-6">{podcast.summary}</p>
            ...
            {!loading && (<footer className="fixed bottom-0 opacity-100 bg-gradient-to-b to-backgroundDark from-backgroundLight/20 left-0 right-0 z-10 pb-8 pt-4 px-4 flex justify-center space-x-4" style={{ paddingBottom: 'calc(1.8rem + env(safe-area-inset-bottom, 24px))' }}>
